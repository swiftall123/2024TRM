...
class ISIC(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.image_folder = join(root, 'train.txt')
        ...
        if self.train == 0:  # train
            ...
            self.image_folder = join(root, 'test.txt')
            self.data_list = []
            self.labels= []
            with open(self.image_folder,'r') as f:
                for line in f:
            ...
            self.image_folder = join(root, 'val.txt')
            self.data_list = []
            self.labels = []
            with open(self.image_folder, 'r') as f:
                for line in f:
        ...
        for f in self.data_list:
            ...
            data_ = f  # 'D:\\ResearchWork\\code\\DataSet\\ISIC\\train/ISIC_0000000.jpeg'
            # print(data_)
            # a = os.path.isfile(data_)  # True
            # assert os.path.isfile(data_)
            if self.device == 1:   # self.device = 0
